...
export const IPFS_MANIFEST_PATH = process.env.IPFS_MANIFEST_PATH;
...
export const startupCheckManifestFile = async () => {
  ...
  globalStartupManifestFileChecks.promise = (async () => {
    try {
      ...
      if (!IPFS_MANIFEST_PATH || IPFS_MANIFEST_PATH.trim() === '') {
        console.info(
          '[startupCheckManifestFile] No IPFS_MANIFEST_PATH specified - skipping manifest file check',
        );
        return { success: true, skipped: true };
      }

      const result = await checkManifestFile(IPFS_MANIFEST_PATH);
