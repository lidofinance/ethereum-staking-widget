import { Modal, Tooltip } from '@lidofinance/lido-ui';
...
const HelpTipAnchor = styled.span`
  display: block;
  margin: 0 auto ${({ theme }) => theme.spaceMap.md}px;
  width: fit-content;
...
export const TradeGuardModal = ({ state, onClose }: TradeGuardModalProps) => {
  const { open, level, messages, oracleVerified, title, helpTip } = state;

  return (
    <Modal open={open} onClose={() => onClose(false)}>
      <TransactionModalContent
        icon={<StageIconFail />}
        title={title ?? TITLE_TEXT[level] ?? TITLE_TEXT.blocked}
        description={
          <>
            {oracleVerified && (
            ...
            {helpTip && (
              <Tooltip placement="top" title={helpTip.text}>
                <HelpTipAnchor>{helpTip.label}</HelpTipAnchor>
              </Tooltip>
            )}
          </>
        }
